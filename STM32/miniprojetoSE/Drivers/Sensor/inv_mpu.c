...
static int set_int_enable(unsigned char enable,struct gyro_state_s *st);
 ...
 *  @return     0 if successful.
 */
int mpu_reg_dump(struct gyro_state_s *st)
{
	unsigned char ii;
	unsigned char data;
 ...
 *  @return     0 if successful.
 */
int mpu_read_reg(unsigned char reg, unsigned char *data,struct gyro_state_s *st)
{
 ...
 *  @return     0 if successful.
 */
int mpu_init(struct int_param_s *int_param,const int AD0,struct gyro_state_s *st)
{
	unsigned char data[6];

	if(AD0 == 0){
		struct gyro_state_s st0 = {
				.reg = &reg,
				.hw = &hw0,
				.test = &test
		};
	...
	//	static struct gyro_state_s st0 = {
	//			.reg = &reg,
	//			.hw = &hw0,
	//			.test = &test
	//	};
	//
	//	static struct gyro_state_s st1 = {
	//			.reg = &reg,
	//			.hw = &hw1,
	//			.test = &test
	//	};
	...
	if(HAL_I2C_IsDeviceReady(&hi2c1,st->hw->addr<<1,2,100)!=HAL_OK)
	{
		log_i("\n\rerror in I2C! (1)\n");
		return -1;
	}
	...
	data[0] = BIT_RESET;
	if (i2c_write(st->hw->addr, st->reg->pwr_mgmt_1, 1, data)!= HAL_OK){
		log_i("\n\rerror in I2C! (2)\n");
		return -1;
	}
	...
	if (i2c_write(st->hw->addr, st->reg->accel_cfg2, 1, data) != HAL_OK)
		return -1;
#endif
	...
	if (mpu_set_gyro_fsr(2000,st))
		return -1;
	if (mpu_set_accel_fsr(2,st))
		return -1;
	if (mpu_set_lpf(42,st))
		return -1;
	if (mpu_set_sample_rate(50,st))
		return -1;
	if (mpu_configure_fifo(0,st))
		return -1;
	...
	if (mpu_set_bypass(0,st))
		return -1;
#endif

	mpu_set_sensors(0,st);
 ...
 *  @return     0 if successful.
 */
int mpu_lp_accel_mode(unsigned short rate,struct gyro_state_s *st)
{
	unsigned char tmp[2];
 ...
 *  @return     0 if successful.
 */
int mpu_get_gyro_reg(short *data, unsigned long *timestamp,struct gyro_state_s *st)
{
	unsigned char tmp[6];
 ...
 *  @return     0 if successful.
 */
int mpu_get_accel_reg(short *data, unsigned long *timestamp,struct gyro_state_s *st)
{
	unsigned char tmp[6];
 ...
 *  @return     0 if successful.
 */
int mpu_get_temperature(long *data, unsigned long *timestamp,struct gyro_state_s *st)
{
	unsigned char tmp[2];
	short raw;
 ...
 *  @return     0 if successful.
 */
int mpu_read_6500_accel_bias(long *accel_bias,struct gyro_state_s *st) {
	unsigned char data[6];
 ...
 *  @return     0 if successful.
 */
int mpu_read_6050_accel_bias(long *accel_bias,struct gyro_state_s *st) {
	unsigned char data[6];
...
int mpu_read_6500_gyro_bias(long *gyro_bias,struct gyro_state_s *st) {
	unsigned char data[6];
 ...
 *  @return     0 if successful.
 */
int mpu_set_gyro_bias_reg(long *gyro_bias,struct gyro_state_s *st)
{
 ...
 *  @return     0 if successful.
 */
int mpu_set_accel_bias_6050_reg(const long *accel_bias,struct gyro_state_s *st) {
	unsigned char data[6] = {0, 0, 0, 0, 0, 0};
	long accel_reg_bias[3] = {0, 0, 0};

	if(mpu_read_6050_accel_bias(accel_reg_bias,st))
		return -1;
 ...
 *  @return     0 if successful.
 */
int mpu_set_accel_bias_6500_reg(const long *accel_bias,struct gyro_state_s *st) {
	unsigned char data[6] = {0, 0, 0, 0, 0, 0};
	long accel_reg_bias[3] = {0, 0, 0};

	if(mpu_read_6500_accel_bias(accel_reg_bias,st))
		return -1;
 ...
 *  @return 0 if successful.
 */
int mpu_reset_fifo(struct gyro_state_s *st)
{
	unsigned char data;
 ...
 *  @return     0 if successful.
 */
int mpu_get_gyro_fsr(unsigned short *fsr,struct gyro_state_s *st)
{
 ...
 *  @return     0 if successful.
 */
int mpu_set_gyro_fsr(unsigned short fsr,struct gyro_state_s *st)
{
	unsigned char data;
 ...
 *  @return     0 if successful.
 */
int mpu_get_accel_fsr(unsigned char *fsr,struct gyro_state_s *st)
{
 ...
 *  @return     0 if successful.
 */
int mpu_set_accel_fsr(unsigned char fsr,struct gyro_state_s *st)
{
	unsigned char data;
 ...
 *  0 if successful.
 */
int mpu_get_lpf(unsigned short *lpf,struct gyro_state_s *st)
{
 ...
 *  @return     0 if successful.
 */
int mpu_set_lpf(unsigned short lpf,struct gyro_state_s *st)
{
	unsigned char data;
 ...
 *  @return     0 if successful.
 */
int mpu_get_sample_rate(unsigned short *rate,struct gyro_state_s *st)
{
	if (st->chip_cfg.dmp_on)
		return -1;
	else
 ...
 *  @return     0 if successful.
 */
int mpu_set_sample_rate(unsigned short rate,struct gyro_state_s *st)
{
	unsigned char data;
	...
	if (st->chip_cfg.dmp_on)
		...
		if (st->chip_cfg.lp_accel_mode) {
			if (rate && (rate <= 40)) {
				...
				mpu_lp_accel_mode(rate,st);
				return 0;
			}
			/* Requested rate exceeds the allowed frequencies in LP accel mode,
			 * switch back to full-power mode.
			 */
			mpu_lp_accel_mode(0,st);
		}
		if (rate < 4)
			rate = 4;
		else if (rate > 1000)
		...
		mpu_set_lpf(st->chip_cfg.sample_rate >> 1,st);
		return 0;
	}
}
 ...
 *  @return     0 if successful.
 */
int mpu_get_gyro_sens(float *sens,struct gyro_state_s *st)
{
	switch (st->chip_cfg.gyro_fsr) {
	case INV_FSR_250DPS:
		sens[0] = 131.f;
 ...
 *  @return     0 if successful.
 */
int mpu_get_accel_sens(unsigned short *sens,struct gyro_state_s *st)
{
	switch (st->chip_cfg.accel_fsr) {
	case INV_FSR_2G:
		sens[0] = 16384;
 ...
 *  @return     0 if successful.
 */
int mpu_get_fifo_config(unsigned char *sensors,struct gyro_state_s *st)
{
 ...
 *  @return     0 if successful.
 */
int mpu_configure_fifo(unsigned char sensors,struct gyro_state_s *st)
{
	unsigned char prev;
	int result = 0;
 ...
 *  @return     0 if successful.
 */
int mpu_get_power_state(unsigned char *power_on,struct gyro_state_s *st)
{
	if (st->chip_cfg.sensors)
		power_on[0] = 1;
	else
 ...
 *  @return     0 if successful.
 */
int mpu_set_sensors(unsigned char sensors,struct gyro_state_s *st)
{
	unsigned char data;
#ifdef AK89xx_SECONDARY
	unsigned char user_ctrl;
 ...
 *  @return     0 if successful.
 */
int mpu_get_int_status(short *status,struct gyro_state_s *st)
{
	unsigned char tmp[2];
	if (!st->chip_cfg.sensors)
		return -1;
...
int mpu_read_fifo(short *gyro, short *accel, unsigned long *timestamp,
		unsigned char *sensors, unsigned char *more,struct gyro_state_s *st)
{
...
int mpu_read_fifo_stream(unsigned short length, unsigned char *data,
		unsigned char *more,struct gyro_state_s *st)
{
	unsigned char tmp[2];
 ...
 *  @return     0 if successful.
 */
int mpu_set_bypass(unsigned char bypass_on,struct gyro_state_s *st)
{
	unsigned char tmp;
 ...
 *  @return     0 if successful.
 */
int mpu_set_int_level(unsigned char active_low,struct gyro_state_s *st)
{
 ...
 *  @return     0 if successful.
 */
int mpu_set_int_latched(unsigned char enable,struct gyro_state_s *st)
{
	unsigned char tmp;
...
#ifdef MPU6050
static int get_accel_prod_shift(float *st_shift,struct gyro_state_s *st)
{
...
static int accel_self_test(long *bias_regular, long *bias_st,struct gyro_state_s *st)
{
	int jj, result = 0;
	float st_shift[3], st_shift_cust, st_shift_var;

	get_accel_prod_shift(st_shift,st);
	for(jj = 0; jj < 3; jj++) {
...
static int gyro_self_test(long *bias_regular, long *bias_st,struct gyro_state_s *st)
{
	int jj, result = 0;
	unsigned char tmp[3];
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test,struct gyro_state_s *st)
{
	unsigned char data[MAX_PACKET_LENGTH];
...
int mpu_run_6500_self_test(long *gyro, long *accel, unsigned char debug)
{
	...
	mpu_get_fifo_config(&fifo_sensors,st);
	...
	mpu_configure_fifo(fifo_sensors,st);
 ...
 *  @return     Result mask (see above).
 */
int mpu_run_self_test(long *gyro, long *accel,struct gyro_state_s *st)
{
#ifdef MPU6050
	const unsigned char tries = 2;
	long gyro_st[3], accel_st[3];
	...
	/* Get initial settings. */
	mpu_get_gyro_fsr(&gyro_fsr,st);
	mpu_get_accel_fsr(&accel_fsr,st);
	mpu_get_lpf(&lpf,st);
	mpu_get_sample_rate(&sample_rate,st);
	sensors_on = st->chip_cfg.sensors;
	mpu_get_fifo_config(&fifo_sensors,st);
...
#if defined MPU6050
	for (ii = 0; ii < tries; ii++)
		if (!get_st_biases(gyro, accel, 0,st))
			break;
	if (ii == tries) {
		/* If we reach this point, we most likely encountered an I2C error.
		 * We'll just report an error for all three sensors.
	...
	for (ii = 0; ii < tries; ii++)
		if (!get_st_biases(gyro_st, accel_st, 1,st))
			break;
	if (ii == tries) {
		/* Again, probably an I2C error. */
		result = 0;
	...
	accel_result = accel_self_test(accel, accel_st,st);
	gyro_result = gyro_self_test(gyro, gyro_st,st);
	...
	st->chip_cfg.fifo_enable = 0xFF;
	st->chip_cfg.clk_src = INV_CLK_PLL;
	mpu_set_gyro_fsr(gyro_fsr,st);
	mpu_set_accel_fsr(accel_fsr,st);
	mpu_set_lpf(lpf,st);
	mpu_set_sample_rate(sample_rate,st);
	mpu_set_sensors(sensors_on,st);
	mpu_configure_fifo(fifo_sensors,st);

	if (dmp_was_on)
		mpu_set_dmp_state(1,st);
...
int mpu_write_mem(unsigned short mem_addr, unsigned short length,
		unsigned char *data,struct gyro_state_s *st)
{
	unsigned char tmp[2];
...
int mpu_load_firmware(unsigned short length, const unsigned char *firmware,
		unsigned short start_addr, unsigned short sample_rate,struct gyro_state_s *st)
{
	unsigned short ii;
...
#define LOAD_CHUNK  (16)
	...
	for (ii = 0; ii < length; ii += this_write) {
		...
		if (mpu_write_mem(ii, this_write, (unsigned char*)&firmware[ii],st)){
			HAL_UART_Transmit(&hlpuart1, (uint8_t *)"Error in mpu_load_firmware (1)\n\r", 33, 100);
			return -1;
		}
		if (mpu_read_mem(ii, this_write, cur,st)){
			HAL_UART_Transmit(&hlpuart1, (uint8_t *)"Error in mpu_load_firmware (2)\n\r", 33, 100);
			return -1;
		}
		if (memcmp(firmware+ii, cur, this_write)){
			//HAL_UART_Transmit(&hlpuart1, (uint8_t *)cur, sizeof(cur), 100);
			HAL_UART_Transmit(&hlpuart1, (uint8_t *)"Error in mpu_load_firmware (3)\n\r", 33, 100);
			return -2;
		}
	}
	...
	if (i2c_write(st->hw->addr, st->reg->prgm_start_h, 2, tmp) != HAL_OK){
		HAL_UART_Transmit(&hlpuart1, (uint8_t *)"Error in mpu_load_firmware(4)\n\r", 40, 100);
		return -1;
	}
 ...
 *  @return     0 if successful.
 */
int mpu_set_dmp_state(unsigned char enable,struct gyro_state_s *st)
{
	unsigned char tmp;
	...
	if (enable) {
		...
		mpu_set_sample_rate(st->chip_cfg.dmp_sample_rate,st);
 ...
 *  @return     0 if successful.
 */
int mpu_get_dmp_state(unsigned char *enabled,struct gyro_state_s *st)
{
 ...
 *  @return     0 if successful.
 */
int mpu_get_compass_reg(short *data, unsigned long *timestamp,struct gyro_state_s *st)
{
#ifdef AK89xx_SECONDARY
	unsigned char tmp[9];
...
int mpu_lp_motion_interrupt(unsigned short thresh, unsigned char time,
		unsigned short lpa_freq,struct gyro_state_s *st)
{
	...
	if (lpa_freq) {
		...
		if (!st->chip_cfg.int_motion_only) {
			...
			mpu_get_gyro_fsr(&st->chip_cfg.cache.gyro_fsr,st);
			mpu_get_accel_fsr(&st->chip_cfg.cache.accel_fsr,st);
			mpu_get_lpf(&st->chip_cfg.cache.lpf,st);
			mpu_get_sample_rate(&st->chip_cfg.cache.sample_rate,st);
			st->chip_cfg.cache.sensors_on = st->chip_cfg.sensors;
			mpu_get_fifo_config(&st->chip_cfg.cache.fifo_sensors,st);
		}
	...
	st->chip_cfg.fifo_enable = 0xFF;
	st->chip_cfg.clk_src = INV_CLK_PLL;
	mpu_set_sensors(st->chip_cfg.cache.sensors_on,st);
	mpu_set_gyro_fsr(st->chip_cfg.cache.gyro_fsr,st);
	mpu_set_accel_fsr(st->chip_cfg.cache.accel_fsr,st);
	mpu_set_lpf(st->chip_cfg.cache.lpf,st);
	mpu_set_sample_rate(st->chip_cfg.cache.sample_rate,st);
	mpu_configure_fifo(st->chip_cfg.cache.fifo_sensors,st);
